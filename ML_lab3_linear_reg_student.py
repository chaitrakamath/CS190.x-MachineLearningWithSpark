...
def parsePoint(line):
    ...
    return LabeledPoint(lineSplits[0], lineSplits[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    sqrErrorRDD = labelsAndPreds.map(lambda x: squaredError(x[0], x[1]))
    sqrErrSumRDD = sqrErrorRDD.reduce(operator.add)
    rmse = math.sqrt(sqrErrSumRDD / labelsAndPreds.count())
    return rmse
...
def gradientSummand(weights, lp):
    ...
    """
    wgtArray = np.array(weights)
    wgtTranspose = np.transpose(wgtArray)
    diff = wgtTranspose.dot(lp.features) - lp.label
    return np.dot(diff, lp.features)
...
def getLabeledPrediction(weights, observation):
    ...
    """
    wgtTranspose = np.transpose(weights)
    prediction = np.dot(wgtTranspose, observation.features)
    result = (observation.label, prediction)
    return result
...
def twoWayInteractions(lp):
    ...
    """
    label = lp.label
    iterProd = list(itertools.product(lp.features, repeat = 2))
    featuresProd =  [x * y for (x, y) in iterProd]
    features = list(np.hstack((lp.features, featuresProd)))
