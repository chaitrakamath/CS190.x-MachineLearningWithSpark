...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats,[(OHEDict[element],1) for element in rawFeats])
...
print parsedTrainFeat.flatMap(lambda x: x).take(20)
...
# TODO: Replace <FILL IN> with appropriate code
ctrOHEDict = createOneHotDict( parsedTrainFeat.flatMap(lambda x: x).distinct())
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    vector =  oneHotEncoding(rawFeats, OHEDict, numOHEFeats)
    return LabeledPoint(point[0], vector)
...
# TODO: Replace <FILL IN> with appropriate code
# train(data, iterations=100, step=1.0, miniBatchFraction=1.0, initialWeights=None, 
#       regParam=0.01, regType='l2', intercept=False, validateData=True)
model0 = LogisticRegressionWithSGD.train(data = OHETrainData, iterations = numIters, regParam = 1e-6, regType = 'l2', step = stepSize,intercept=True)
...
#   Now check value of y and return result accordingly
    if y == 1:
        return (-log(p))      
    else:
        return (-log(1-p))
...
classOneFracTrain = OHETrainData.filter(lambda x: x.label == 1.0).map(lambda x: x.label).sum() / OHETrainData.count()
print classOneFracTrain

# print OHETrainData.map(lambda x: x.label).take(5)
...
def evaluateResults(model, data):
    ...
    """
#     predictedVal = data.map(lambda x: getP(x.features, model.weights, model.intercept)).collect()
...
def parseHashPoint(point, numBuckets):
    ...
    sparseFeats = SparseVector(numBuckets, hashFunction(numBuckets, features, printMapping=False))
...
def computeSparsity(data, d, n):
    ...
    """
    sparsity = data.map(lambda x: float(len(x.features.indices))).reduce(lambda x,y: x+y) / (d * n)  
    return sparsity
